...
> "In past pairwise comparisons, when the first tire had this directional package relative to the second tire, it won with the listed historical win rate."
...
The long-term purpose is to support a tire development assistant that can reason about which directional design changes have historically accompanied improved snow braking performance within comparable test requests.


## 2. Data Structure and Comparison Unit

A `req_no` represents one test request that usually contains 2-6 tires, often around 3 tires. Because snow braking tests can have high variability across requests, this analysis only compares tires within the same `req_no`.

For example, if one request contains tires A, B, and C, the pairwise dataset contains:
- A vs B
- A vs C
- B vs C

Therefore, multiple rows can share the same `req_no`. The row-level evidence should be interpreted as within-request directional comparisons, not as fully independent test events.

This within-REQ comparison design is intentional: snow braking measurements can vary substantially across test requests, so comparisons across different requests may mix design effects with test-condition variation.


## 3. How To Read the Direction Terms
...
- `HS`: Hysteresis. This is the compound energy-loss characteristic under deformation. 
...
## 5. Selected Historical Packages

The historical win rate is the share of rows where the first tire won when it had the listed directional package relative to the second tire.

The listed win rate is row-weighted: among pairwise rows where the first tire had the listed directional package relative to the second tire, it is the share of rows where the first tire was the actual snow braking winner.

### (1) Pattern and edge feature packages

| Package | Historical win rate | pairwise rows / unique req_no | Interpretation and development insight |
|---|---:|---:|---|
